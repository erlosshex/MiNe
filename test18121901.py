...
def copy_image_from_path_to_path(src_path,dst_path,copy_file_list):
    #copy_file_list = file_list[index_list]
    for this_file in copy_file_list:
        old_file_path = src_path + '/' + this_file
        new_file_path = dst_path + '/' + this_file
        shutil.copyfile(old_file_path,new_file_path)

def find_last(ss,s):
    last_position = -1
    while True:
        pos = str.find(s,last_position+1)
        if pos == -1:
...
def create_train_valid_no_labels():
    ...
    copy_image_from_path_to_path(positive_image_path,save_file_path+'/'+pre_train_file_str,train_positive_file_name)
    copy_image_from_path_to_path(positive_image_path,save_file_path+'/'+pre_valid_file_str,valid_positive_file_name)
    copy_image_from_path_to_path(negative_image_path,save_file_path+'/'+pre_train_file_str,train_negative_file_name)
    copy_image_from_path_to_path(negative_image_path,save_file_path+'/'+pre_valid_file_str,valid_negative_file_name)
...
def create_train_valid_with_labels():
    ...
    if param_according_image_flag == True:
        train_positive_file_name = positive_file_name[train_positive_index_list]
        valid_positive_file_name = positive_file_name[valid_positive_index_list]
        train_negative_file_name = negative_file_name[train_negative_index_list]
        valid_negative_file_name = negative_file_name[valid_negative_index_list]
        copy_image_from_path_to_path(positive_image_path,save_file_path+'/'+pre_train_file_str,train_positive_file_name)
        copy_image_from_path_to_path(positive_image_path,save_file_path+'/'+pre_valid_file_str,valid_positive_file_name)
        copy_image_from_path_to_path(negative_image_path,save_file_path+'/'+pre_train_file_str,train_negative_file_name)
        copy_image_from_path_to_path(negative_image_path,save_file_path+'/'+pre_valid_file_str,valid_negative_file_name)
        ...
        train_positive_labels_list = positive_label_name[train_positive_index_list]
        valid_positive_labels_list = positive_label_name[valid_positive_index_list]
        train_negative_labels_list = negative_label_name[train_negative_index_list]
        valid_negative_labels_list = negative_label_name[valid_negative_index_list]
        write_image_name_to_file(pre_train_text_str,train_positive_labels_list)
        write_image_name_to_file(pre_train_text_str,train_negative_labels_list)
        write_image_name_to_file(pre_valid_text_str,valid_positive_labels_list)
        write_image_name_to_file(pre_valid_text_str,valid_negative_labels_list)
